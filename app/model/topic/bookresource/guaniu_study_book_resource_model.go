...
// arModel is a active record design model for table guaniu_study_book_resource operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of guaniu_study_book_resource.
	Table = "guaniu_study_book_resource"
	// Model is the model object of guaniu_study_book_resource.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table guaniu_study_book_resource.
	Columns = struct {
		Id              string // 编号
		BookName        string // 书的名称
		BookAuthor      string // 书的作者
		BookDesc        string // 书的描述
		BookTags        string // 书的标签
		BookCbs         string // 出版社
		PrintTime       string // 发行时间
		BookIsbn        string // isbn
		Status          string // 是否开放状态（0正常 1 删除）
