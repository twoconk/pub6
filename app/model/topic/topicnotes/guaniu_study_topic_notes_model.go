...
// arModel is a active record design model for table guaniu_study_topic_notes operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of guaniu_study_topic_notes.
	Table = "guaniu_study_topic_notes"
	// Model is the model object of guaniu_study_topic_notes.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table guaniu_study_topic_notes.
	Columns = struct {
		Id              string // 编号
		TopicId         string // 主题id
		UserId          string // 用户id
		TaskTitle       string // 笔记标题
		TaskContent     string // 笔记描述
		LikeNum         string // 喜欢数
		OrderNum        string // 显示顺序
		Status          string // 是否开放状态（0免费 1 收费  2 只对自己可见 3 只有关注者可见 ）
