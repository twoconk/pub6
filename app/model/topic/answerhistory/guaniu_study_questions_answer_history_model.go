...
// arModel is a active record design model for table guaniu_study_questions_answer_history operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of guaniu_study_questions_answer_history.
	Table = "guaniu_study_questions_answer_history"
	// Model is the model object of guaniu_study_questions_answer_history.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table guaniu_study_questions_answer_history.
	Columns = struct {
		Id            string // 编号
		TopicId       string // 主题id
		UserId        string // 用户id
		Status        string // 是否开放状态（0开放  1  只对自己可见 3 只有关注者可见 ）
