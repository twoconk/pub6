...
// arModel is a active record design model for table guaniu_study_topic_members operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of guaniu_study_topic_members.
	Table = "guaniu_study_topic_members"
	// Model is the model object of guaniu_study_topic_members.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table guaniu_study_topic_members.
	Columns = struct {
		Id              string // 编号
		TopicId         string // 主题id
		UserId          string // 成员id
		Status          string // 是否开放状态（0正常 1 禁言）
		AdminFlag       string // 是否管理员（0不是 1 管理员）
